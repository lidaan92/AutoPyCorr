...
# Unzips and untars B8.tifs from Landsat 7 scenes from /Landsat7/downloads
# And moves them to their corresponding folders in /Landsat7/antarctica/pxxx_rxxx
...
print "================================= Moving Landsat7 downloaded files ================================\n";

##################################################### log file ##################################################
print "Operations will be logged in /Landsat7/logs 'move_L7dl.log' +-----------------------------------]|>\n";
open my $fh, ">>", "/Volumes/RAID/Landsat7/logs/move_L7dl.log" or die "FATAL: Could not open log!: $!\n";
print $fh "=========================================================================================\n";
...
chdir "/Volumes/RAID/Landsat7/downloads" or die "FATAL: Failed to cd!: $!";

system ("rm -v *\\(1\\)*");    #removes all duplicates in directory

#open(STDERR, '>>', "/Volumes/RAID/Landsat7/logs/move_error.log") or die "failed to open move_error.log!: $!";
...
unless (@files) {
    die "x==========(ง ͠° ͟ل͜ ͡°)ง=============x> No files here. Exiting! <x============(ง ͠° ͟ل͜ ͡°)ง============x\n";
}

for my $tar_file (@tar_files) {
    my ($path, $row) = $tar_file =~ m/^LE7(\d{3})(\d{3})\d+\w+\d\d\.tar.gz$/xms;
...
############################################# path variables #################################################    
    ...
    my $dirpath = "/Volumes/RAID/Landsat7/antarctica/$dir";
...
################################################## tar #######################################################     
	...
	print $fh "--> Moved $tif to $band8_path\n"
      ...
      print "logging missing $tif to missing_b8.log in '/Landsat7/logs'\n";
      open my $b8fh, ">>", "/Volumes/RAID/Landsat7/logs/missing_b8.log";
      print $b8fh "----------------------------\n";
      print $b8fh "$tif\n";
      print $b8fh "$raw_path";
      close $b8fh;
...
##############################################################################################################     
    ...
    {
     chdir "/Volumes/RAID/Landsat7/downloads";
     print "---------------------------Remaining tars in /Landsat7/downloads------------------------------\n";
     system ("ls -G *.tar*\n");
     print "______________________________________________________________________________________________\n";
    }
}

print "================= tar files done! ======└(°ᴥ°)┘=========...now jpgs...=============================\n";

for my $jpg_file (@jpg_files) {
    my ($path, $row) = $jpg_file =~ m/^LE7(\d{3})(\d{3})\d+\w+\d\d\.jpg$/xms;
...
##############################################################################################################    
    ...
    chdir "/Volumes/RAID/Landsat7/downloads";
     print "-------------------------- Remaining jpgs in /Landsat7/downloads -----------------------------\n";
     system ("ls -G\n");
     print "______________________________________________________________________________________________\n";
...
print "[̲̅$̲̅(̲̅ ͡° ͜ʖ ͡°̲̅)̲̅$̲̅]\n";
print "===+===+======+===+======+===+=====┻━┻===+===+======+===+======+===+======+===+======+===+===\n";
print "-つ◕ل͜◕)つ---It's been a long battle ... but I think most of us made it out alive ---ヽ( ͝° ͜ʖ͡°)ﾉ -----\n";
print "^*===><===**===><===*===><===**===><===*===><===*^*===><===*===><===**===><===*===><===**===><===*^\n";
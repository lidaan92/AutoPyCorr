...
##	    out_name_base=img1.filename[0:20]+'_'+img2.filename[0:20]+'_'+str(midpoint)+'_hp'+str(args.gfilt_sigma)
##	else:
##	    out_name_base=args.out_name_base
...
##	    os.makedirs(out_parent_dir)
	...
	if(args.out_name_base=='temp_out'and args.use_hp):
                out_parent_dir=args.imgdir+'/../'+out_name_base+'_'+str(int(del_t_days))+'_hp/'
        else:
            out_parent_dir=args.imgdir+'/../'+out_name_base+'_'+str(int(del_t_days))+'/'
        if not os.path.exists(out_parent_dir):
	    os.makedirs(out_parent_dir)               
...
for i in range(min_ind_i,max_ind_i+1):
	for j in range(min_ind_j,max_ind_j+1):
		...
		if(test_for_masked_pix>0.0):  # there are all greater than zero pixels, so out of mask (valid original image can't have pixel values less than or equal to zero...)
			numcorr+=1
			chip_tar=img2_src_arr[(r_j_2[j]-half_target_chip):(r_j_2[j]+half_target_chip),(r_i_2[i]-half_target_chip):(r_i_2[i]+half_target_chip)]
...
if not(args.no_gtif):   ###### This flag is not presently used, as GTiff forms basis for all output right now...
	...
	if(args.kmz):
		...
		sp.call(["rm", file_name_base+"_4326.tif", "tmp.kml"])  # clear older tries
		...
		sp.check_call(["unzip", file_name_base+".kmz", "tmp.kml"])
		...
		sp.check_call(["zip", "-u", file_name_base+".kmz", "tmp.kml", file_name_base+"_colorbar.png"])
	...
	if(args.log10):
		...
		if(args.kmz):
			...
			plt.savefig(outcbname)
			sp.call(["rm", file_name_base+"_log10_4326.tif", "tmp.kml"])  # clear older tries
			...
			sp.check_call(["unzip", file_name_base +"_log10.kmz", "tmp.kml"])
			in_kml_tree=ET.parse('tmp.kml')
			root=in_kml_tree.getroot()
			b=ET.Element(root.tag[:-3]+'Folder')  # make a new node to be the root folder - the tag drops the "kml" (-3) but keeps the namespace url...
			b.attrib['id']=file_name_base  # the name of the folder you see in GE
			...
			newtree=ET.ElementTree(root)
			newtree.write('tmp.kml')
			sp.check_call(["zip", "-u", file_name_base+"_log10.kmz", "tmp.kml", file_name_base+"_log10_colorbar.png"])
			sp.check_call(["rm", "tmp.kml"])  # done with modified tmp.kml, erase it
			if not(args.nlf):
